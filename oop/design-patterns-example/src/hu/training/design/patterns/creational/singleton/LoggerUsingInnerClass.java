...
public class LoggerUsingInnerClass {
	 ...
	 * by the class loader) than the moment that getInstance() is called. Thus,
	 * this solution is thread-safe without requiring special language
	 * constructs (i.e. volatile or synchronized).
	 * 
	 * To sum up, LoggerHolder is loaded on the first execution of
	 * LoggerUsingInnerClass.getInstance() or the first access to
	 * LoggerHolder.INSTANCE, not before.
	 */
